  ...
  Public Class DBMTests
    ...
    Private Shared Function TestCase(InputPoint As String,
      ...
      With Result
        Return Hash({Round(.Factor, 3), Round(.OriginalFactor, 3), _
          ...
          Round(.AbsoluteErrorStats.ModifiedCorrelation, 4), _
          Round(.AbsoluteErrorStats.Determination, 4)}) = _
    ...
    Private Shared Function UnitTestResults As Boolean
      ...
      UnitTestResults = UnitTestResults And _
        ...
        Hash(RemoveOutliers({3009.2004, 3779.3677, 3500.9705, 3114.8486, _
        3312.1548, 3478.4153, 3080.0972, 2848.6597, 3197.1477, 3064.1587, _
        3100.6992, 2953.3582})) = Hash({3009.2004, NaN, 3500.9705, 3114.8486, _
        3312.1548, 3478.4153, 3080.0972, 2848.6597, 3197.1477, 3064.1587, _
        3100.6992, 2953.3582})
      ...
      With Statistics
        For i = 0 To 19
          If i = 0 Then
            .Calculate({3411, 3067, 3159, 2579, 2604, 3549, 2028, 3521, 3629, _
            3418, 2091, 2828})
          ElseIf i = 1 Then
            .Calculate({3725, 3581, 2747, 3924, 3743, 2112, 3899, 2728, 3050, _
            3534, 2107, 3185})
          ElseIf i = 2 Then
            .Calculate({2937, 2596, 3245, 3296, 2528, 2559, 3660, 3649, 3178, _
            3972, 3822, 2454})
          ElseIf i = 3 Then
            .Calculate({3390, 3960, 2488, 3068, 2213, 3999, 3352, 2031, 3150, _
            2200, 2206, 3598})
          ElseIf i = 4 Then
            .Calculate({2569, 2091, 2592, 2764, 2602, 3897, 3960, 2803, 2557, _
            2321, 2326, 3293})
          ElseIf i = 5 Then
            .Calculate({2820, 2826, 3425, 2652, 3266, 2415, 2372, 3167, 2161, _
            2916, 3811, 2523})
          ElseIf i = 6 Then
            .Calculate({3570, 2758, 2579, 3839, 3263, 3255, 2857, 2196, 3122, _
            3389, 3827, 3670})
          ElseIf i = 7 Then
            .Calculate({2045, 3087, 3832, 2861, 3356, 3005, 3027, 2926, 2707, _
            2810, 2539, 2111})
          ElseIf i = 8 Then
            .Calculate({2488, 3958, 2122, 2781, 2730, 2980, 2311, 2949, 2515, _
            3258, 3084, 2313})
          ElseIf i = 9 Then
            .Calculate({3877, 3309, 3012, 2781, 2215, 3568, 2919, 3507, 3192, _
            3665, 2038, 2421})
          ElseIf i = 10 Then
            .Calculate({2148, 2211, 2663, 2256, 2000, 3074, 3314, 3088, 3655, _
            2164, 2384, 3358}, {3, 5, 10, 20, 28, 32, 41, 46, 57, 66, 74, 76})
          ElseIf i = 11 Then
            .Calculate({2908, 2714, 2300, 3409, 3858, 3060, 2179, 3515, 2804, _
            2924, 2984, 2415}, {10, 18, 28, 35, 44, 50, 51, 62, 63, 66, 74, 80})
          ElseIf i = 12 Then
            .Calculate({2659, 2191, 3180, 2340, 3855, 2196, 2888, 2546, 3745, _
            3501, 2546, 3347}, {9, 11, 13, 18, 21, 26, 28, 37, 42, 47, 56, 61})
          ElseIf i = 13 Then
            .Calculate({2513, 2180, 2062, 2645, 3580, 2595, 2471, 2961, 2509, _
            2681, 2090, 2965}, {6, 14, 23, 26, 32, 35, 37, 41, 45, 48, 58, 68})
          ElseIf i = 14 Then
            .Calculate({2412, 3729, 3177, 3510, 3856, 2662, 3086, 2161, 3269, _
            2820, 3921, 2229}, {7, 12, 23, 29, 30, 32, 36, 38, 41, 49, 55, 61})
          ElseIf i = 15 Then
            .Calculate({3847, 3240, 2695, 2298, 2960, 2439, 3987, 2261, 2058, _
            2691, 3095, 3846}, {4, 9, 15, 18, 20, 26, 36, 45, 49, 58, 64, 71})
          ElseIf i = 16 Then
            .Calculate({3076, 2813, 3694, 3652, 3345, 3444, 3994, 2680, 2990, _
            2826, 3391, 2358}, {7, 14, 24, 28, 37, 40, 49, 51, 55, 61, 69, 77})
          ElseIf i = 17 Then
            .Calculate({2846, 3086, 3629, 3082, 2855, 3018, 2456, 3238, 2980, _
            3362, 3773, 2741}, {6, 16, 23, 29, 35, 40, 49, 60, 64, 73, 75, 78})
          ElseIf i = 18 Then
            .Calculate({2605, 2586, 2301, 3060, 2447, 3169, 2727, 3752, 2956, _
          ...
          UnitTestResults = UnitTestResults And _
            ...
            Round(.OriginSlope, 4) = {383.2213, 397.5889, 426.4229, _
            371.4506, 374.8399, 372.6621, 425.6739, 359.6522, 360.8676, _
            379.3893, 52.1812, 50.6373, 75.2409, 60.0661, 73.5505, 61.2832, _
            59.1633, 53.9967, 46.4227, 66.0541}(i) And _
            ...
            Round(.OriginAngle, 4) = {89.8505, 89.8559, 89.8656, _
            ...
            Round(.StandardError, 4) = {582.4218, 633.706, 535.0359, _
            720.9024, 619.3358, 506.8629, 525.9328, 483.3154, 527.9273, _
            573.7699, 544.1683, 523.5486, 590.2042, 436.8994, 644.5969, _
            698.2517, 478.875, 384.1275, 419.8051, 657.4656}(i) And _
            Round(.Correlation, 4) = {-0.1548, -0.374, 0.3411, _
            ...
            Round(.ModifiedCorrelation, 4) = {0.819, 0.7932, 0.8652, _
            0.7909, 0.8474, 0.8345, 0.8554, 0.8061, 0.8274, 0.7962, 0.8665, _
            0.9037, 0.8892, 0.908, 0.887, 0.8274, 0.8714, 0.8915, 0.9047, _
            0.8561}(i) And _
            Round(.Determination, 4) = {0.024, 0.1398, 0.1164, 0.082, _
            0.0271, 0.0004, 0.0509, 0.1184, 0.0063, 0.1413, 0.1845, 0.0001, _
