...
Namespace Vitens.DynamicBandwidthMonitor
  ...
  Public Class DBMTests
    ...
    Public Shared Sub RunUnitTests
      ...
      AssertEqual(StDev({60}), 0)
      AssertEqual(StDev({72}), 0)
      AssertEqual(StDev({32, 95}), 31.5)
      AssertEqual(StDev({81, 75}), 3)
      AssertAlmostEqual(StDev({67, 76, 25}), 22.2261)
      AssertEqual(StDev({25, NaN, 27}), 1)
      AssertAlmostEqual(StDev({31, 73, 83, 81}), 21.1187)
      AssertAlmostEqual(StDev({18, 58, 47, 47}), 14.8408)
      AssertAlmostEqual(StDev({10, NaN, 20, 30, NaN}), 8.165)
      AssertAlmostEqual(StDev({67, 90, 74, 32, 62}), 19.0158)
      AssertAlmostEqual(StDev({78, 0, 98, 65, 69, 57}), 30.2072)
      AssertAlmostEqual(StDev({49, 35, 74, 25, 28, 92}), 24.7437)
      AssertAlmostEqual(StDev({7, 64, 22, 7, 42, 34, 30}), 18.6537)
      AssertAlmostEqual(StDev({13, 29, 39, 33, 96, 43, 17}), 25.5223)
      AssertAlmostEqual(StDev({59, 78, 53, 7, 18, 44, 63, 40}), 21.9986)
      AssertAlmostEqual(StDev({77, 71, 99, 39, 50, 94, 67, 30}), 23.1864)
      AssertAlmostEqual(StDev({91, 69, 63, 5, 44, 93, 89, 45, 50}), 27.035)
      AssertAlmostEqual(StDev({12, 84, 12, 94, 52, 17, 1, 13, 37}), 31.9575)
      AssertAlmostEqual(StDev({18, 14, 54, 40, 73, 77, 4, 91, 53, 10}), 29.4354)
      AssertAlmostEqual(StDev({80, 30, 1, 92, 44, 61, 18, 72, 63, 41}), 27.1654)
    ...
    Public Shared Function RunQualityTests As String
      ...
      For Week = 1 To 52
        With Statistics(DBM.GetResults(InputPointDriver, Nothing,
          New DateTime(2016, 1, 1).AddWeeks(Week-1),
          New DateTime(2016, 1, 1).AddWeeks(Week), 0,
